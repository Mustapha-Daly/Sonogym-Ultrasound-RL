...
class USSlicer(LabelImgSlicer):
    ...
    def __init__(
        ...
        # ------------------------------------------------------------
        # FIX: Never allow a boolean to overwrite the visualize() method.
        # Store the flag under a different name.
        # ------------------------------------------------------------
        self.enable_visualization = bool(visualize)

        # ------------------------------------------------------------
        # IMPORTANT: prevent base class from creating self.visualize = bool
        # by forcing visualize=False in super().__init__
        # ------------------------------------------------------------
        super().__init__(
            label_maps,
            ct_maps,
    ...
    def construct_T_maps(self):
        ...
        self.T_rand_frame_poses = torch.stack(self.T_rand_frame_poses, dim=0)
        self.T_rand_frame_poses_aug = self.T_rand_frame_poses[self.env_to_human_inds, :]
    ...
    def construct_Vl_maps(self):
        ...
        self.Vl_rand_frame_poses = torch.stack(self.Vl_rand_frame_poses, dim=0)
        self.Vl_rand_frame_poses_aug = self.Vl_rand_frame_poses[self.env_to_human_inds, :]
        self.Vl_rand_frame_poses_aug = self.Vl_rand_frame_poses_aug.reshape((-1, 1, 3))
    ...
    def slice_US(
        ...
        # ... UNCHANGED (your original slice_US) ...
        ...
        if self.sim_mode == "conv" or self.sim_mode == "both":
            ...
            ct_img_tensor = self.ct_img_tensor.permute(0, 3, 1, 2)
            if ct_img_tensor.shape[1] > 1:
    ...
    def visualize(self, key, first_n=10):
        # ------------------------------------------------------------
        # FIX: Use the dedicated flag; do NOT allow overwrite of method name.
        # ------------------------------------------------------------
        ...
        if key == "US":
            ...
            plt.imshow(
                (
                    ...
                    / (np.max(combined_img_np) - np.min(combined_img_np) + 1e-8)
                    * 255
                ).astype(np.uint8),
                cmap="gray",
            )
            ...
            if self.sim_mode == "both":
                ...
                plt.clf()
                plt.imshow(
                    (combined_img_np_net.T / (np.max(combined_img_np_net) + 1e-8) * 255).astype(np.uint8),
                    cmap="gray",
                )
                plt.pause(0.0001)
