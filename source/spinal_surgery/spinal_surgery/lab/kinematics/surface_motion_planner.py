...
class SurfaceMotionPlanner(HumanFrameViewer):
    ...
    def __init__(self, label_maps, human_list, num_envs, x_z_range, init_x_z_x_angle, device, roll_adj=0.0, 
        ...
        for i in range(self.n_human_types):
            highest_y_array_path = human_list[i] + '/body_highest_y_array.pt'
            if os.path.exists(highest_y_array_path):
                surface_map = torch.load(highest_y_array_path, map_location=device)
            else:
                surface_map = construct_highest_y_array(self.label_maps[i], self.body_label)
                torch.save(surface_map, highest_y_array_path)
            self.surface_map_list.append(surface_map)

        # construct surface normal list X * Z * 3: surface normal at 2d position
        self.surface_normal_list = []
        for i in range(self.n_human_types):
            surface_normal_array_path = human_list[i] + '/body_surface_normal_array_highest.pt'
            if os.path.exists(surface_normal_array_path):
                surface_normal = torch.load(surface_normal_array_path, map_location=device)
            else:
                surface_normal = construct_boundary_normals_array_aligned(
                    self.label_maps[i],
                    self.surface_map_list[i],
                    self.body_label,
                    align_y=-1.0,
                )
