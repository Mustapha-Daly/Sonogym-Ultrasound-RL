...
def construct_highest_y_array(label_map, label):
    X, Y, Z = label_map.shape
    highest_y_array = torch.full((X, Z), -1, dtype=torch.int64, device=label_map.device)
    for x in range(X):
        for z in range(Z):
            y_indices = torch.nonzero(label_map[x, :, z] == label, as_tuple=False)
            if len(y_indices) > 0:
                highest_y = y_indices.max().item()
                highest_y_array[x, z] = highest_y
...
def construct_boundary_normals_array_aligned(label_map, y_array, label, align_y=1.0, smoothing=True):
    ...
    normals_array[:, :, 1] = align_y
    x_grid, z_grid = torch.meshgrid(
        torch.arange(X, device=label_map.device),
        torch.arange(Z, device=label_map.device),
        indexing="ij",
    )
    ys = y_array[x_grid, z_grid]
    valid = ys >= 0
    normals_array[x_grid[valid], z_grid[valid]] = normals[x_grid[valid], ys[valid], z_grid[valid]]
    target_axis = torch.tensor([0.0, align_y, 0.0], device=label_map.device)
    normal_norms = torch.linalg.norm(normals_array, dim=-1)
    inner_products = normals_array @ target_axis
    normals_array[inner_products < 0, :] = -normals_array[inner_products < 0, :]
    normals_array[normal_norms < 1e-8, :] = target_axis
