#adapted env from vertebra to kidney navigation
...
class roboticUSEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: roboticUSEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 0 = searching
        # 1 = scanning (spine found)
        # 2 = done (end anatomy, e.g. kidney)
    ...
    def _get_observations(self) -> dict:
        ...
        vertebra_visible = (label == self.cfg.VERTEBRA_LABEL_ID).any(dim=(1, 2))
        kidney_visible   = (label == self.cfg.KIDNEY_LABEL_ID).any(dim=(1, 2))
        
        self._vertebra_visible = vertebra_visible
        self._kidney_visible = kidney_visible

        start_mask = vertebra_visible & (self.scan_state == 0)
        self.scan_state[start_mask] = 1

        end_mask = kidney_visible & (self.scan_state == 1)
        ...
        if self.num_step % 30 == 0:
            ...
            if self.observation_mode == "US":
                ...
                palette[15] = torch.tensor([0,0,200], device=label2d.device)  # main organ class
                ...
                legend_items = [
                    ...
                    mpatches.Patch(color=(0,0,0.6), label="Kidney (ID 15)")
                ]
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        kidney_mask = (label == self.cfg.KIDNEY_LABEL_ID)
        B, H, W = kidney_mask.shape
        kidney_pixels = kidney_mask.sum(dim=(1, 2))
        kidney_fraction = kidney_pixels / (H * W)
        # initialize previous value
        if not hasattr(self, "prev_kidney_fraction"):
            self.prev_kidney_fraction = kidney_fraction.clone()
        # compute progress
        progress = kidney_fraction - self.prev_kidney_fraction
        # clamp progress to avoid unstable spikes
        progress = torch.clamp(progress, min=-0.05, max=0.05)

        reward = (2.0 * kidney_fraction + 4.0 * progress)
    
        #reward = kidney_fraction  #i added
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.num_step % 50 == 0:
            wandb.log({
                "kidney_fraction": kidney_fraction.mean().item()
            })   
        terminated = (kidney_fraction > 0.20)
